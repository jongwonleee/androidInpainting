...
public class Inpaint {
    ...
    public Bitmap inpaint(Bitmap input, Bitmap mask, int radius) {
        // initial image
       ...
       //if(mask!=null) return mask;
        ...
        Log.i("Inpainting Info","["+source.W+"*"+source.H+"] build pyramid of images...");
    ...
    private MaskedImage ExpectationMaximization(int level) {
        ...
        Log.i("Inpainting Info","ExpectationMaximization Level:"+level+" done.");
        ...
        for (int y = 0; y < target.H; y++) {
            for (int x = 0; x < target.W; x++) {
                ...
                for (int band = 0; band < 3; band++) {
                    ...
                    rgb[band] = (int) (contrib / wcontrib);
                }
                target.setSample(x, y, rgb[0], rgb[1], rgb[2]);
