package com.jongwonleee.androidinpainting.inpainting;
...
/**
 * Wrapper/Helper for Masked RGB Bitmap
 *
...
public class MaskedImage {
    ...
    // construct from existing Bitmap and mask
    public MaskedImage(Bitmap image, boolean[][] mask) {
        this.image = image;
    ...
    public int getSample(int x, int y, int band) {
        ...
        if (band == 0) val = Color.red(pixel);
        else if (band == 1) val = Color.green(pixel);
        else if (band == 2) val = Color.blue(pixel);
        return val;
    }

    public void setSample(int x, int y, int r, int g, int b) {
        int pixel = image.getPixel(x, y);
    ...
    public static int distance(MaskedImage source, int xs, int ys, MaskedImage target, int xt, int yt, int S) {
        ...
        for (int dy = -S; dy <= S; dy++) {
            for (int dx = -S; dx <= S; dx++) {
                ...
                if (xks < 0 || xks >= source.W) {
                    distance += ssdmax;
                    continue;
                }
                if (yks < 0 || yks >= source.H) {
                    distance += ssdmax;
                    continue;
                }
                ...
                if (xkt < 0 || xkt >= target.W) {
                    distance += ssdmax;
                    continue;
                }
                if (ykt < 0 || ykt >= target.H) {
                    distance += ssdmax;
                    continue;
                }
    ...
    public static Bitmap resize(Bitmap input, int newwidth, int newheight) {
        ...
        Canvas g = new Canvas(out);
        Bitmap scaled = Bitmap.createScaledBitmap(input, newwidth, newheight, true);
        g.drawBitmap(scaled, 0f, 0f, new Paint());
        return out;
    }
    ...
    public MaskedImage copy() {
        ...
        Bitmap newimage = Bitmap.createBitmap(image, 0, 0, W, H);
    ...
    public MaskedImage downsample() {
        ...
        for (int y = 0, ny = 0; y < H - 1; y += 2, ny++) {
            for (int x = 0, nx = 0; x < W - 1; x += 2, nx++) {
                ...
                if (ksum > 0) {
                    newimage.setSample(nx, ny, (int) ((double) r / ksum + 0.5), (int) ((double) g / ksum + 0.5), (int) ((double) b / ksum + 0.5));
